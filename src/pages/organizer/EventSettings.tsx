...
export default function EventSettings() {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(rawModules)) modulesArray = rawModules;
      ...
      modulesArray = (rawModules as any)
        .split(",")
        .map((name: string) => ({
          id: name.trim(),
          name: name.trim(),
      ...
      modulesArray = Object.entries(rawModules).map(([key, val]) =>
        typeof val === "object"
          ? { id: key, name: key, ...val }
          : { id: key, name: key, enabled: !!val },
      );
    }
  ...
  const doSave = async () => {
    ...
    try {
      ...
      const payload: any = {
        ...
        reply_to_email:
          formData.replyToEmail && formData.replyToEmail.trim()
            ? formData.replyToEmail.trim()
            : formData.fromEmail && formData.fromEmail.trim()
  ...
  if (!rawEvent && error)
    return (
      <div className="py-16 text-center text-destructive">
        Error loading event: {String(error.message)}
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle style={{ fontSize: "var(--font-h3)", fontWeight: 600 }}>
              Event Modules
            </CardTitle>
          </CardHeader>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {comingSoonModules.map((module) => {
                ...
                return (
                  <div
                    key={module.id}
                    className="rounded-lg border p-3 opacity-50 cursor-not-allowed border-border"
                  >
                    <div className="flex items-center justify-between mb-2">
