...
export default function PromoEmbed() {
  ...
  const {
    ...
    queryFn: () => {
      const qs = intakeStatus
        ? `?intake_status=${encodeURIComponent(intakeStatus)}`
        : "";
  ...
  const speakerList: Speaker[] = React.useMemo(() => {
    ...
    if (Array.isArray((rawSpeakers as any).items))
      return (rawSpeakers as any).items as Speaker[];
    if (Array.isArray((rawSpeakers as any).results))
      return (rawSpeakers as any).results as Speaker[];
    if (Array.isArray((rawSpeakers as any).speakers))
      return (rawSpeakers as any).speakers as Speaker[];
    if (Array.isArray((rawSpeakers as any).data))
      return (rawSpeakers as any).data as Speaker[];
    const firstArray = Object.values(rawSpeakers).find((v: any) =>
      Array.isArray(v),
    );
      ...
      <div className="mx-auto max-w-6xl">
        {isLoading ? (
          ...
          <p className="text-red-600">
            Failed to load speakers: {String(error.message)}
          </p>
          ...
          <div className="flex flex-wrap gap-6 justify-center">
            {visibleSpeakers.map((speaker: any) => {
              ...
              const promoTemplate =
                ...
                eventData?.promoCardTemplate ??
                null;
              const companyLogo =
                speaker.companyLogo ?? speaker.company_logo ?? null;
                  ...
                  <div className="relative w-full h-full flex items-end justify-center text-center p-6 overflow-hidden">
                    {headshot && (
                      <img
                        ...
                        className="absolute rounded-lg object-cover border-4 border-white z-10"
                      />
                    )}
                    <div
