...
import {
  ...
  DEFAULT_FIELDS,
  mergeWithDefaults,
} from "@/components/SpeakerFormBuilder";
...
const DUPLICATE_EMAIL_MESSAGE =
  "A speaker with this email already exists for this event.";
...
export default function SpeakerIntakeForm(
  props: { formPageType?: "speaker-intake" | "call-for-speakers" } = {},
) {
  ...
  const [contentItems, setContentItems] = useState<any[]>([
    { id: "content-1", file: null, preview: null, name: "", contentType: null },
  ]);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitted, setSubmitted] = useState(false);
  const [submittedSpeakerId, setSubmittedSpeakerId] = useState<string | null>(
    null,
  );
  ...
  const [formConfig, setFormConfig] =
    useState<FormFieldConfig[]>(DEFAULT_FIELDS);
  ...
  const pageType: "speaker-intake" | "call-for-speakers" =
    formPageType ??
    (location.pathname.includes("/call-for-speakers")
      ? "call-for-speakers"
      : location.pathname.includes("/speaker-intake")
        ? "speaker-intake"
        : "speaker-intake");

  const backendFormType =
    pageType === "speaker-intake" ? "speaker-info" : "call-for-speakers";
          ...
          .then((res: any) => {
            ...
            try {
              ...
              if (Array.isArray(res.config)) {
                ...
                const show =
                  typeof res.showTitle === "boolean"
                    ? res.showTitle
                    : (res.metadata?.showTitle ?? true);
  ...
  // Prefill fields from URL query params when present (e.g. sent via organiser email)
  React.useEffect(() => {
    const q = new URLSearchParams(location.search);
    const fName =
      q.get("firstName") ?? q.get("first_name") ?? q.get("first") ?? null;
    const lName =
      q.get("lastName") ?? q.get("last_name") ?? q.get("last") ?? null;
    const email = q.get("email") ?? null;
    const readonlyFlag = q.get("readonly_email") ?? null;
    const resetValues: Record<string, any> = {};
    let needReset = false;
    if (fName) {
      resetValues["first_name"] = fName;
      needReset = true;
    }
    if (lName) {
      resetValues["last_name"] = lName;
      needReset = true;
    }
    if (email) {
  ...
  // We use this to get the image crop shapes (e.g. headshot should be circular, company logo should be landscape) for the ImageCropDialog
  const { data: websiteConfig } = useQuery({
    queryKey: ["event", eventId, "websiteConfig"],
    queryFn: async () =>
      getJson<any>(
        `/promo-cards/config/${encodeURIComponent(eventId!)}?promo_type=website`,
      ),
  ...
  React.useEffect(() => {
    ...
    formConfig.forEach((f) => {
      if (f.id === "first_name")
        resetData[f.id] = s.firstName ?? s.first_name ?? "";
      else if (f.id === "last_name")
        resetData[f.id] = s.lastName ?? s.last_name ?? "";
      else if (f.id === "email") resetData[f.id] = s.email ?? "";
      else if (f.id === "company_name")
        resetData[f.id] = s.companyName ?? s.company_name ?? "";
      else if (f.id === "company_role")
        resetData[f.id] = s.companyRole ?? s.company_role ?? "";
  ...
  React.useEffect(() => {
    ...
    const originalEmail = String((fetchedSpeaker as any)?.email ?? "")
      .trim()
      .toLowerCase();
    ...
    if (
      !cleanEmail ||
      !isValidEmail ||
      (isEditing && originalEmail && cleanEmail === originalEmail)
    ) {
      emailCheckRequestRef.current += 1;
      setExistingEmailError(null);
      setIsCheckingExistingEmail(false);
    ...
    emailCheckTimeoutRef.current = window.setTimeout(async () => {
      try {
        const existsResult = await checkSpeakerExistsForEvent(
          eventId,
          cleanEmail,
        );
  ...
  const handleCropComplete = async (croppedBlob: Blob) => {
    try {
      ...
      const mime =
        (croppedBlob as Blob & { type?: string }).type || "image/jpeg";
      const rawExt = mime.includes("/") ? mime.split("/")[1] : "jpeg";
      const ext =
        rawExt.split("+")[0] === "jpeg" ? "jpg" : rawExt.split("+")[0];
      ...
      reader.onloadend = () => {
        if (isHeadshot) {
          ...
          setCustomFilePreviews((prev) => ({
            ...prev,
            [cropType]: reader.result as string,
          }));
        }
        const label = isHeadshot
          ...
          : isLogo
            ? "Company logo"
            : (formConfig.find((f) => f.id === cropType)?.label ?? "Image");
  ...
  const CROP_FIELD_IDS = new Set([
    "headshot",
    "company_logo",
    "company_logo_white",
  ]);
  const GENERAL_ACCEPT =
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      const contentFieldEnabled = formConfig.some(
        (f) => f.enabled && f.id === "sample_content",
      );
      if (contentFieldEnabled) {
        const missingName = contentItems.some(
          (it) => it.file && !it.name.trim(),
        );
        if (missingName) {
      ...
      const submittedEmail = String(data.email ?? "")
        .trim()
        .toLowerCase();
      const isValidSubmittedEmail = /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(
        submittedEmail,
      );
      const originalEmail = String((fetchedSpeaker as any)?.email ?? "")
        .trim()
      ...
      if (
        emailFieldEnabled &&
        submittedEmail &&
        isValidSubmittedEmail &&
        !(isEditing && originalEmail && submittedEmail === originalEmail)
      ) {
        const existsResult = await checkSpeakerExistsForEvent(
          eventId,
          submittedEmail,
      ...
      const speakerDisplayName =
        `${data.first_name ?? payload.firstName ?? ""} ${data.last_name ?? payload.lastName ?? ""}`.trim();
      ...
      if (headshot) {
        try {
          ...
          const headshotUrl =
            res?.public_url ?? res?.publicUrl ?? res?.url ?? null;
      ...
      const otherFileFields = formConfig.filter(
        (f) =>
          f.enabled &&
          f.type === "file" &&
          f.id !== "headshot" &&
          ...
          if (it.file) {
            try {
              ...
              payload.content.push({
                content: url,
                contentType:
                  it.contentType || it.file.type || "application/octet-stream",
      ...
      if (
        ...
        const email = String(
          form.getValues("email" as any) ?? "",
        ).trim();
        const qs = email
          ? `?speakerEmail=${encodeURIComponent(email)}`
          : "";
        const loginUrl = `${window.location.origin}/login${qs}`;
        ...
        // Use the public event-level mail endpoint so this confirmation can
        // be sent without requiring a speaker-specific resource to exist.
        emailEvent(eventId, {
          recipientEmail,
      ...
      // After a successful submission redirect to the login page so the
      // speaker can sign in and finish setting up their profile.
      const email = String(
        form.getValues("email" as any) ?? "",
      ).trim();
      const qs = email
  ...
  // Only fetch event details on pages where the user is authenticated.
  // This is a public intake form — avoid hitting a protected `/events/:id`
  // endpoint and causing 401s when no token is present.
  const { data: eventData, isLoading: loadingEvent } = useQuery({
    queryKey: ["event", eventId],
    queryFn: () => getJson<any>(`/events/${eventId}`),
    enabled: Boolean(eventId && getCurrentToken()),
  });
  ...
  const handleLoginEmailChange = React.useCallback((email: string) => {
    ...
    if (loginCheckTimeoutRef.current)
      window.clearTimeout(loginCheckTimeoutRef.current);
  ...
  if (submitted && pageType === "call-for-speakers") {
    return (
      ...
      >
        <div
          className="w-[90%] max-w-[440px] bg-card rounded-lg p-12 text-center"
        ...
        >
          ...
          <h2 className="text-xl font-semibold mb-2">
            ...
            {resolvedEventName ? ` to ${resolvedEventName}` : ""}
          </h2>
            ...
            <Form {...form}>
              {enabledFields.map((field) => {
                  ...
                  return (
                    <div key={field.id} className="space-y-1.5">
                      ...
                      <ContentUploads
                        ...
                        readOnly={false}
                        showValidation={contentValidationShown}
                      />
                    </div>
                  );
                }
                ...
                if (field.type === "file") {
                  ...
                  const preview = isHeadshot
                    ? headshotPreview
                    : isLogo
                      ? companyLogoPreview
                      : (customFilePreviews?.[field.id] ?? null);
                  const uploading = isHeadshot
                    ? uploadingHeadshot
                    : isLogo
                      ? uploadingLogo
                      : false;
                  const isImagePreview =
                    isHeadshot || isLogo
                      ? !!preview
                      : (preview?.startsWith("data:image") ?? false);
                  return (
                    <div key={field.id} className="space-y-1.5">
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          {field.type === "textarea" ? (
                            ...
                            // For email field, respect prefilled read-only mode so organisers can lock the address
                            <Input
                              className={"text-sm " + (field.id === "email" && prefilledEmail ? "bg-muted/50 cursor-not-allowed" : "")}
                              placeholder={field.placeholder}
                              {...formField}
                              readOnly={field.id === "email" && prefilledEmail}
                            />
                          )}
                        </FormControl>
                        <FormMessage />
                        ...
                        {field.id === "email" && prefilledEmail && (
                          <p className="text-s text-accent mt-1">
                            Need to change your email? Contact the team at{" "}
                            {resolvedEventName ?? "the event"}
                          </p>
                        )}
                      </FormItem>
                    )}
            ...
            {cropImageUrl && (
              <ImageCropDialog
                ...
                title={
                  cropType === "headshot"
                    ? "Crop Headshot"
                    : `Crop ${formConfig.find((f) => f.id === cropType)?.label ?? "Logo"}`
                }
                cropShape={(() => {
                  // Default fallbacks
                  if (cropType === "headshot") {
                    return (
                      (websiteConfig?.config?.headshot?.shape as any) ??
                      "square"
                    );
                  }
                  if (cropType === "logo") {
                    return (
                      (websiteConfig?.config?.companyLogo?.shape as any) ??
                      "square"
                    );
                  }
                  return "square";
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-destructive">
              A speaker with this email already exists for this event. Please
              log in and update your profile from the speaker dashboard instead
              of submitting a new intake form.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction
