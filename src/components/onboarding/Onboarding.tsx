...
const Onboarding: React.FC = () => {
  ...
  const [state, setState] = useState<OnboardingState>(() => {
    // Restore current step if user was redirected for integrations (preserve progress)
    let restoredStep = OnboardingStep.ORGANIZATION;
    try {
      const saved = localStorage.getItem("onboarding.currentStep");
      const n = saved ? parseInt(saved, 10) : NaN;
      if (
        !Number.isNaN(n) &&
        n >= OnboardingStep.ORGANIZATION &&
        n <= OnboardingStep.SUCCESS
      )
        restoredStep = n as OnboardingStep;
    } catch (e) {
      // ignore
    }
        ...
        if (status?.connected) {
          ...
          setState((prev) => ({
            ...
            event: {
              ...
              rootFolder:
                (status as any).root_folder ??
                (folders.length
                  ? folders[0].id
                  : (prev.event.rootFolder ?? "")),
              integrationId:
                (status as any).integration?.id ??
                (status as any).integration_id ??
  ...
  // Persist current onboarding step so external OAuth redirects can restore progress
  React.useEffect(() => {
    try {
      localStorage.setItem("onboarding.currentStep", String(state.currentStep));
    } catch (e) {
      // ignore
    }
  }, [state.currentStep]);

  // On mount, restore saved draft (form data) and ensure step is restored after OAuth
  React.useEffect(() => {
    try {
    ...
    try {
      ...
      if (saved) {
        ...
        if (!Number.isNaN(n) && n >= OnboardingStep.ORGANIZATION && n <= OnboardingStep.SUCCESS) {
          setState((prev) => ({ ...prev, currentStep: n as OnboardingStep }));
        }
      }
    } catch (e) {
      // ignore
  ...
  // Autosave draft to localStorage (exclude files)
  React.useEffect(() => {
    try {
      const draft = JSON.stringify({ organization: state.organization, team: state.team, event: state.event });
      localStorage.setItem('onboarding.draft', draft);
    } catch (e) {
      // ignore
    }
  }, [state.organization, state.team, state.event]);
    ...
    if (state.currentStep === OnboardingStep.ORGANIZATION) {
      ...
      try {
        ...
        const teamRes = await createTeam({
          ...
          description: state.team.purpose,
        ...
        try {
          if (eventImageFile) {
            ...
            const imageValue =
              up?.public_url ?? up?.publicUrl ?? up?.url ?? up?.id ?? null;
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-2">
      <div className="w-full max-w-4xl h-[calc(100vh-4rem)] flex flex-col justify-between">
        ...
        <main className="mt-8 flex-1">
          ...
          {state.currentStep === OnboardingStep.EVENT && (
            <StepContainer
              ...
              isNextDisabled={
                !state.event.title || !state.event.startDate || state.isLoading
              }
            >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {state.event.googleDriveConnected ? (
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              // Persist current step before redirecting to external OAuth
                              try {
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Select value={state.team.name ? "current-team" : ""}>
                        <SelectTrigger className="w-full sm:w-[300px]" disabled>
                          <SelectValue
                            placeholder={state.team.name || "Team"}
                          />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="current-team">
                            {state.team.name || "Your Team"}
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateEvent({ startDate: e.target.value })
                          }
                          required
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateEvent({ endDate: e.target.value })
                          }
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateEvent({ eventWebsite: e.target.value })
                        }
                      />
                    </div>
                  </CardContent>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateEvent({ fromName: e.target.value })
                        }
                      />
                    </div>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateEvent({ fromEmail: e.target.value })
                          }
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateEvent({ replyToEmail: e.target.value })
                          }
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        onChange={(e) =>
                          updateEvent({ emailSignature: e.target.value })
                        }
                        rows={3}
                      />
                    </div>
                  ...
                  <CardContent>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                      {availableModules.map((module) => {
                        ...
                        const isSelected = (state.event.modules || []).includes(
                          module.id,
                        );
        ...
        <Dialog
          ...
          onOpenChange={(v) => setShowSkipConfirm(Boolean(v))}
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Skip onboarding?</DialogTitle>
              <DialogDescription>
                Some features may not work as expected if you skip onboarding.
                You can finish setup later from the dashboard, but we'll take
                you there now.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
        ...
        <footer className="mt-16 flex items-start justify-center text-gray-400 text-sm max-w-lg mx-auto text-center space-x-2">
          ...
          <p>
            This flow helps you get started with Seamless Events in just a few
            minutes.
          </p>
        </footer>
      </div>
