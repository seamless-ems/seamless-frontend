...
function resolveAppStatus(speaker: any) {
  ...
  if (s === "rejected")
    return {
      label: "Rejected",
      cls: "bg-transparent text-foreground border-destructive/40",
    };
  if (s === "submitted")
    return {
      label: "Submitted",
...
export function EmailComposer({
  ...
  const handleCopy = async () => {
    const html = buildEmailHtml(
      draft.speaker.firstName,
      draft.body,
      intakeUrl ?? "",
      draft.ctaLabel,
      draft.signOff,
    );
    const from = [draft.fromName, draft.fromEmail ? `<${draft.fromEmail}>` : ""]
      .filter(Boolean)
      .join(" ");
    const ctaLine =
          ...
          <div className="rounded-t-lg border border-border bg-muted/20 px-4">
              ...
              <Input
                ...
                onChange={(e) => {
                  localStorage.setItem(
                    "seamless-email-from-name",
                    e.target.value,
                  );
                  onDraftChange({ fromName: e.target.value });
                }}
            ...
            <div className={EMAIL_HEADER_ROW}>
              ...
              <Input
                ...
                onChange={(e) => {
                  localStorage.setItem(
                    "seamless-email-from-email",
                    e.target.value,
                  );
                  onDraftChange({ fromEmail: e.target.value });
                }}
          ...
          <div className="rounded-b-lg border border-t-0 border-border bg-white px-8 pt-5 pb-4 space-y-3">
            ...
            <div className="border-t border-gray-100 pt-3 text-center">
              <span className="text-[11px] text-gray-400">
                Powered by <span className="font-medium">Seamless Events</span>
              </span>
            </div>
          </div>
        </div>
...
export default function ApplicationsTab({
  ...
  const { data: rawApplications, isLoading } = useQuery<any>({
    ...
    queryFn: () =>
      getJson<any>(`/events/${eventId}/speakers?form_type=call-for-speakers`),
  ...
  const allApplications: any[] = (() => {
    ...
    else if (Array.isArray(rawApplications.results))
      arr = rawApplications.results;
    else if (Array.isArray(rawApplications.speakers))
      arr = rawApplications.speakers;
    ...
    return arr.map((it: any) => ({
      ...
      callForSpeakersStatus:
        it.call_for_speakers_status ?? it.callForSpeakersStatus ?? "pending",
      submittedAt: it.registered_at ?? it.created_at ?? it.createdAt ?? null,
      name:
        `${it.first_name ?? it.firstName ?? ""} ${it.last_name ?? it.lastName ?? ""}`.trim() ||
        (it.email ?? ""),
    }));
  })();
  ...
  const counts = {
    ...
    submitted: allApplications.filter(
      (a) => a.callForSpeakersStatus === "submitted",
    ).length,
    pending: allApplications.filter(
      (a) => a.callForSpeakersStatus === "pending",
    ).length,
    approved: allApplications.filter(
      (a) => a.callForSpeakersStatus === "approved",
    ).length,
    rejected: allApplications.filter(
      (a) => a.callForSpeakersStatus === "rejected",
    ).length,
  };
  ...
  const handleSendEmail = async (
    ...
    try {
      const html = buildEmailHtml(
        draft.speaker.firstName ?? draft.speaker.name ?? "",
        draft.body,
        intakeUrl ?? "",
        draft.ctaLabel,
        draft.signOff,
      ...
      toast({
        title: "Email sent",
        description: `Message sent to ${payload.recipientEmail}`,
      });
      queryClient.invalidateQueries({
        queryKey: ["event", eventId, "applications"],
      });
    ...
    <div className="pt-6">
          ...
          <thead className="border-b border-border bg-muted/20">
            <tr className="h-11">
              ...
              <th className="px-4 py-2 text-right">
                ...
                >
                  <p>
                    Share the form link so speakers can apply. Approve to move
                    them to your{" "}
                    <span className="font-medium text-foreground">
                      Speakers
                    </span>{" "}
                    tab; reject to dismiss.
                  </p>
                </HelpTip>
              </th>
            </tr>
          ...
          <tbody>
            {isLoading ? (
              ...
              filtered.map((applicant) => {
                ...
                const isApproved =
                  applicant.callForSpeakersStatus === "approved";
                const isRejected =
                  applicant.callForSpeakersStatus === "rejected";
                const isApproving = approvingId === applicant.id;
                const subtitle = [applicant.companyRole, applicant.company]
                  .filter(Boolean)
                  .join(" · ");
                ...
                return (
                  <tr
                    ...
                    onClick={() =>
                      (window.location.href = `/organizer/event/${eventId}/speakers/${applicant.id}`)
                    }
                  >
                    ...
                    >
                      <div className="flex items-center justify-end gap-2">
                        {(isApproved || isRejected) && (
                          <Badge
                            variant="outline"
                            className={`text-xs font-medium whitespace-nowrap ${status.cls}`}
                          >
                            {status.label}
                          </Badge>
                        )}
      ...
      {approvalEmail && (
        <EmailComposer
          ...
          onDraftChange={(patch) =>
            setApprovalEmail((e) => (e ? { ...e, ...patch } : null))
          }
          onSend={async () => {
      ...
      {rejectionEmail && (
        <EmailComposer
          ...
          onSend={async () => {
            await handleSendEmail(rejectionEmail);
