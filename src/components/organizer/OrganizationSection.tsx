...
export default function OrganizationSection() {
  ...
  const form = useForm<{ name: string; domain?: string; description?: string }>(
    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-4">
                ...
                <div key={o.id} className="space-y-2 border p-3 rounded">
                  ...
                  <div>
                    ...
                    <div className="font-medium">
                      {o.createdBy?.email || o.createdBy?.name || "—"}
                    </div>
                  </div>
                  {o.users && o.users.length > 0 && (
                    <div>
                      ...
                      <div className="font-medium">
                        {o.users.map((u: any) => u.email || u.name).join(", ")}
                      </div>
                    </div>
                  )}
                </div>
          ...
          {creating ? (
            <form
              onSubmit={form.handleSubmit((vals) => createMut.mutate(vals))}
            ...
            <div>
              <div className="text-sm text-muted-foreground">
                {orgs && orgs.length > 0
                  ? "You can create additional organizations for your account."
