...
## April 9 — Card rendering — per-speaker text logic

`cfg.fontSize` is the **maximum**. Apply per speaker at render time:

- **`firstName` / `lastName` (separate elements — used by all current templates):** Each must render on **exactly 1 line**. Shrink `cfg.fontSize` (1px at a time, min 20px) until the text fits within `cfg.width` without wrapping. Do NOT let either element wrap — this breaks the fixed y-positions of title/company below. `cfg.fontSize` is the maximum; the actual rendered size may be smaller per-speaker.
- **Name (legacy `name` element):** `nameFormat` is always `"two-line"`. Split: all words except the last on line 1, last word on line 2. Shrink font (1px at a time, min 20px) until both lines fit within `cfg.width`.
...
4. **Send a notification email** to the speaker confirming their application was accepted and including the Speaker Intake link (`{origin}/speaker-intake/{eventId}`) so they can log in or create a free Seamless account and complete their profile (headshot, bio, and any missing fields).
